...
class Security extends \Dao\Table
{
    static public function newUsuario($email, $password)
    {
        ...
        //ESTO ES TEMPORAL
        $pruebaTrim =preg_split('//', $email, -1, PREG_SPLIT_NO_EMPTY);
        $randomNum = rand(01,999);
        $pruebaUsuarioAuto = $pruebaTrim[0].$pruebaTrim[1].$pruebaTrim[2].$pruebaTrim[3].$pruebaTrim[4].$pruebaTrim[5].$randomNum;
        //ESTO ES TEMPORAL


        $newUser["useremail"] = $email;
        // $newUser["username"] = "John Doe"; //ESTE ES EL ORIGINAL
        $newUser["username"] = $pruebaUsuarioAuto; //ESTO ES TEMPORAL
