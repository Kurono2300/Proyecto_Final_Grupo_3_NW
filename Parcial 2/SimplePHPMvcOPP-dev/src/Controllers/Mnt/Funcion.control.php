...
class Funcion extends \Controllers\PublicController
{
    ...
    public function run() :void
    {
        ...
        if (!$this->isPostBack()) {
            ...
            if (!$this->hasErrors) {
                switch ($this->mode){
                case "INS":
                    if (\Dao\Mnt\Funciones::insert($this->fncod, $this->fndsc, $this->fnest, $this->fntyp)) {
    ...
    private function _loadPostData()
    {
        ...
        $this->fntyp = isset($_POST["fntyp"]) ? $_POST["fntyp"] : "ACT" ;


        //validaciones
        //aplicar todas la reglas de negocio
        if (preg_match('/^\s*$/', $this->fncod)) {
            $this->aErrors[] = "¡La funcion no puede ir vacia!";
        }
        //
    ...
    private function _setViewData()
    {
        ...
        $this->fntyp_ACT = ($this->fntyp === "ACT") ? "selected" : "";
        $this->fntyp_INA = ($this->fntyp === "INA") ? "selected" : "";
