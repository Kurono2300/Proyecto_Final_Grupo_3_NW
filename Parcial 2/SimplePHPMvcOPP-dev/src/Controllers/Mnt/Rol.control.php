...
class Rol extends \Controllers\PublicController
{
    ...
    public function run() :void
    {
        ...
        if (!$this->isPostBack()) {
            ...
            if (!$this->hasErrors) {
                switch ($this->mode){
                case "INS":
                    if (\Dao\Mnt\Roles::insert($this->rolescod, $this->rolesdsc, $this->rolesest)) {
                ...
                case "UPD":
                    ...
                    break;
                case "DEL":
                    if (\Dao\Mnt\Roles::delete($this->usercod)) {
                        \Utilities\Site::redirectToWithMsg(
                            "index.php?page=mnt_roles",
        ...
        \Views\Renderer::render("mnt/rol", $dataview);
    }
    ...
    private function _loadPostData()
    {
        ...
        if (preg_match('/^\s*$/', $this->rolescod)) {
            $this->aErrors[] = "¡El Rol no puede ir vacio!";
        }
        //
